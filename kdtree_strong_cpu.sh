...
printf '%s,%s,%s,%s,%s,%s\n' 'n_procs' 'n_threads' 'time_taken' 'mpi_times' 'omp_times' 'problem_size' > results/strong_scaling_cpu_bycore.csv
...
do
    ...
    do  
        ...
        printf '%s,%s,%s,%s,%s,%s\n' ${n} ${j} ${times[j]} ${mpi_times[j]} ${omp_times[j]} ${ps} >> results/strong_scaling_cpu_bycore.csv 
    done
done
